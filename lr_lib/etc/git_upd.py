...
VersionChanges = collections.OrderedDict({
    ...
    'v11.7.1': '''
* исправлен алгоритм поиска Ord, ранее неверно вычислял Ord для больших длин между LB
    ''',

    'v11.7.0': '''
* Исправлены ошибочные выводы warning окон, типа: Snapshot=t.inf, в котором расположен, первый заменяемый param ...
* Убраны ошибочно расположенные raise, которые приводили в остановке работы, вместо её продолжения.
    ''',

    'v11.6.9': '''
* Исправлена проблема с запуском утилиты - автоопределение директории скрипта.
* При открытии, открывает все  vuser_init.c/action.c/.../vuser_end.c файлы, раньше только action.c. Сохраняет все в один файл.
    ''',
...
* fix 11.6.7
    ...
    'v11.6.7': '''
* Найти и создать web_reg_save_param, в аспекте того, что он может обновиться/переопределиться по мере выполнения теста. Если {param}, используется в нескольких web, и его можно создать используя несколько разных web, \nто попытатся создать отдельный wrsp, для неодинаковых wrsp, для param из разных web.
    ''',
...
* обновлен help
    ...
    'v11.6.5': '''
* добавлен help: lr_lib/LR_help.docx
