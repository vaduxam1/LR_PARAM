...
_HELP = '''
Не особо актуальный хелп, вероятно от версии 7.0
...
Описание:
    ...
    A) Где происходит поиск(2) param(1):
        ...
        Формирование AllFiles, чекбоксами[Определить Файлы, для поиска(2)], происходит двумя способами:
         a) Только файлы, прописанные в t*.inf файлах каталога LoadRunner-скрипта, каждому файлу соответствует определенный inf-номер.
            ...
            inf-номер - это номер("Snapshot=t43.inf") блока запроса в LoadRunner-скрипте, перед которым нужно вставлять web_reg_save_param.
            inf-номерами можно ограничить диапазон поиска файлов.
         b) Все файлы каталога, у всех файлов inf-номер=0.
            Номер("Snapshot=t43.inf") блока запроса в LoadRunner-скрипте, перед которым нужно вставлять web_reg_save_param, Не определяется, определять вручную, по имени файла.
    ...
    B) Результат поиска(2) param(1):
        Формируется FilesWithParam список файлов, в которых param(1) был найден.
         a) Список найденых файлов записывается в комбобокс(3), выбирается один из файлов(3) (чекбокс "reverse"), происходит чтение файла(3).
...
web_reg_save_param/Одиночный:
  ...
  Результат [Р]:
    ...
    7) в action.c, показано окно релультатов замены, пока открыто, отображен созданный web_reg_save_param.
        после закрытия окна, выполнен переход, на первый замененный param.
...
CODE = '''Соответствие defaults.Var's с шагами : | <<(на этом шаге используются Vars.get()) | [=> соответствующий gui виджет]
    LR_7.py -> files.init() | <<(VarFilesFolder, VarIsSnapshotFiles, VarAllowDenyFiles, VarEncode, VarAllFilesStatistic, FileOptionsStartswith, DENY_FILES, DENY_PART_NAMES, DENY_EXT)
        |
...
TRANSLIT = {
    ...
    'а': "a", 'б': "b", 'в': "v", 'г': "g", 'д': "d", 'е': "e", 'ё': "e", 'ж': "j", 'з': "z", 'и': "i", 'й': "y",
    'к': "k", 'л': "l", 'м': "m", 'н': "n", 'о': "o", 'п': "p", 'р': "r", 'с': "s", 'т': "t", 'у': "u", 'ф': "f",
    'х': "h", 'ц': "ts", 'ч': "ch", 'ш': "sh", 'щ': "sch", 'ъ': "y", 'ы': "yi", 'ь': "", 'э': "e", 'ю': "yu", 'я': "ya",
}

TRANSLIT_DT = {ord(k): TRANSLIT[k] for k in TRANSLIT}
