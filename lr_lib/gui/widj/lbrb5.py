...
class LBRBText(tk.Text):
    ...
    def __init__(self, name: str, parent: 'lr_lib.gui.wrsp.main_window.Window'):
        ...
        self.label_info = tk.LabelFrame(
            parent, text=self.info_text[name], font=(lr_vars.DefaultFont + ' bold italic'),
        ...
        super().__init__(
            self.label_info, height=lr_vars.DEFAULT_LB_RB_MIN_HEIGHT,
            background=lr_lib.core.var.vars_highlight.Background,
            font=lr_vars.DefaultLBRBFont,
            wrap=tk.NONE,
            padx=0, pady=0,
        )
        self.name = name
